...
public class AuthHelper {
    ...
    public static ConfidentialClientApplication getConfidentialClientInstance() throws MalformedURLException {
        ...
        try {
            final IClientSecret secret = ClientCredentialFactory.createFromSecret(Constants.SECRET);
            confClientInstance = ConfidentialClientApplication.builder(Constants.CLIENT_ID, secret)
    ...
    public static void redirectToSignOutEndpoint(IdentityContextAdapter contextAdapter) throws IOException {
        ...
        final String redirect = String.format("%s%s%s%s", Constants.AUTHORITY, Constants.SIGN_OUT_ENDPOINT,
                Constants.POST_SIGN_OUT_FRAGMENT, URLEncoder.encode(Constants.HOME_PAGE, "UTF-8"));
    ...
    public static void acquireTokenSilently(IdentityContextAdapter contextAdapter)
        ...
        final SilentParameters parameters = SilentParameters.builder(Collections.singleton(Constants.SCOPES), context.getAccount())
                .build();
    ...
    private static void redirectToAuthorizationEndpoint(IdentityContextAdapter contextAdapter) throws IOException {
        ...
        AuthorizationRequestUrlParameters parameters = AuthorizationRequestUrlParameters
                .builder(Constants.REDIRECT_URI, Collections.singleton(Constants.SCOPES)).responseMode(ResponseMode.QUERY)
    ...
    public static void processAADCallback(IdentityContextAdapter contextAdapter) throws AuthException {
        ...
        try {
            ...
            final AuthorizationCodeParameters authParams = AuthorizationCodeParameters
                    .builder(authCode, new URI(Constants.REDIRECT_URI)).scopes(Collections.singleton(Constants.SCOPES))
                    .build();
 ...
 /*   private static void handleGroupsOverage(IdentityContextAdapter contextAdapter) {
    ...
    private static void validateState(IdentityContextAdapter contextAdapter) throws AuthException {
        ...
        if (sessionState == null || requestState == null || !sessionState.equals(requestState)
                || context.getStateDate().before(new Date(now.getTime() - (Constants.STATE_TTL * 1000)))) {
